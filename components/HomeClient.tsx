...
export function HomeClient({
  ...
  const resolvedInitialSlug = useMemo(() => {
    ...
    // Legacy aliases mapped to real registry slugs:
    if (initialTest === 'display' || initialTest === 'screen-test' || initialTest === 'display-test') return 'display-patterns';
    if (initialTest === 'battery' || initialTest === 'battery-test') return 'battery-monitor';
  ...
  const currentTool: ToolDefinition = useMemo(() => {
    ...
    if (!found) {
      console.warn(`[DeviceTry] Unknown test slug "${activeToolSlug}" — falling back to first tool.`);
      return TOOLS_REGISTRY[0];
    }
    return found;
  }, [activeToolSlug]);
    ...
    <div className="space-y-8">
      ...
      {activeTab === 'tests' && (
        <div className="space-y-6">
          ...
          {/* Active Diagnostic Tool (header + tester + guidance) */}
          <ToolDetailView tool={currentTool} t={t} locale={locale} compact titleHeading="h2" />
        </div>
      )}
